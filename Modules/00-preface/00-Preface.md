...
- Open a `.lean` file in VS Code, run `#eval` and `#check`, and read the **Lean Infoview** panel.
...
When writing a tactic proof, the Lean Infoview pane displays:
- **Hypothesis Context** (above `⊢`): All variables, assumptions, and previously proven facts available in the current scope (e.g., `h1 : a ≤ b`).
- **Goal State** (below `⊢`): The exact statement remaining to be established (e.g., `⊢ a ≤ c`).
- **Completion Marker**: When all subgoals are resolved, Lean displays `Goals accomplished 🎉`.
...
3. **Type one command**, exactly: `#eval 2 + 2`
4. **Put the text cursor on that line.** Lean reports about the code *at the cursor* — this is the habit to build now.
5. **Open the Infoview** if it isn't already visible: command palette (`Ctrl+Shift+P` / `Cmd+Shift+P`) → *Lean 4: InfoView: Toggle InfoView*. The panel opens beside the editor and shows `4`.
